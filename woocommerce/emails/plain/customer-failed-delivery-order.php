...
echo sprintf( esc_html__( 'Unfortunately, the delivery for your order #%s has failed.', 'delivium' ), esc_html( $order->get_id() ) ) . "\n\n";

$delivium_driver_id = $order->get_meta('delivium_driverid');
if ( '' !== $delivium_driver_id ) {
	$driver = new DELIVIUM_Driver();
	if ( '' !== $delivium_driver_id ) {
		echo $driver->get_driver_info__premium_only( $delivium_driver_id, 'text' );
		echo $driver->get_vehicle_info__premium_only( $delivium_driver_id, 'text' );
	}
}
...
if ( '' !== $delivium_driver_note ){
	echo esc_html( __( 'Driver note', 'delivium' ) ) . ': ' . $delivium_driver_note . "\n";
}
 
// Signature
$delivium_order_signature = $order->get_meta('delivium_order_last_signature');
	if ( '' !== $delivium_order_signature ) {
	echo esc_html( __( 'Signature', 'delivium' ) ) . ': ';
	echo '<a href="' . esc_attr( $delivium_order_signature ) . '" target="_blank">' . $delivium_order_signature . '</a>';
}

// Photo
$delivium_order_delivery_image = $order->get_meta('delivium_order_last_delivery_image');
if ( '' !== $delivium_order_delivery_image ) {
	echo esc_html( __( 'Delivery image', 'delivium' ) ) . ': ';
	echo '<a href="' . esc_attr( $delivium_order_delivery_image ) . '" target="_blank">' . $delivium_order_delivery_image . '</a>';
}
